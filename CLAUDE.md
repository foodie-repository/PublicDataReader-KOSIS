...
- `{모듈}/{모듈}.py` → 데이터 수집 및 변환 스크립트
- `{모듈}/{모듈}.ipynb` → Plotly 기반 시각화 노트북
- `csv/{모듈}_시도별_YYYYMMDD.csv` → 원본 데이터 (타임스탬프 자동 생성)
- `csv/{모듈}_피벗_전체기간_최종.csv` → 분석용 피벗 테이블

### CSV 저장 경로
모든 CSV 파일은 프로젝트 루트의 `csv/` 폴더에 저장됩니다:
```
csv/
├── 착공실적_시도별_YYYYMMDD.csv
├── 착공실적_피벗_전체기간_최종.csv
├── 인허가실적_시도별_YYYYMMDD.csv
├── 인허가실적_피벗_전체기간_최종.csv
├── 준공실적_시도별_YYYYMMDD.csv
├── 준공실적_피벗_전체기간_최종.csv
├── 미분양현황_시군구별_YYYYMMDD.csv
├── 미분양현황_피벗_전체기간_최종.csv
├── 준공_후_미분양_시군구별_YYYYMMDD.csv
├── 준공_후_미분양_피벗_전체기간_최종.csv
└── 미분양_종합.csv
```
